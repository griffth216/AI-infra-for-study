...
def gemm_residual_partial_rmsnorm(
    ...
    """
    D = torch.addmm(C, A, B, out_dtype=torch.float32)
...
def gemm_rmsnorm(
    ...
    """
    D = torch.mm(A, B, out_dtype=torch.float32)
...
def gemm_partial_cross_entropy(
    ...
    """
    logits = torch.mm(A, B, out_dtype=torch.float32)
...
def gemm_residual_partial_rmsnorm_bwd(
    ...
    D = torch.mm(A, B.T, out_dtype=torch.float32)
    ZdZ = rearrange(ZdZ, "m -> m 1")
    W = rearrange(W, "n -> 1 n")
    R = rearrange(R, "m -> m 1")
    C_norm = C * R
    O = O + ((D * W - C_norm * ZdZ) * R).to(dtype=O.dtype)
    C_out = C_norm * W
...
def gemm_partial_swiglu_bwd(
    ...
    Z2 = rearrange(Z.to(dtype=torch.float32), "... (n pair) -> ... n pair", pair=2)
    G = Z2[..., 0]
    U = Z2[..., 1]
